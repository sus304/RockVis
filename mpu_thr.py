...
def gyro_convert(digit):
  rps = digit / 65.5 # [rad/s]
  return rps
def acc_convert(digit):
  G = digit / 2100.0 * 9.80665 # [m/s2]
  return G
...
longitude = 140.0 # [deg]
length_rail = 5.0 # [m]
file_row = {'temp':0, 'pressure':1, 'acc_axis':4, 'acc_lug':2, 'acc_side':3}
#################################################  
...
acc_y_log = acc_convert(all_log_original[:, file_row['acc_lug']]) # [m/s2]
acc_z_log = acc_convert(all_log_original[:, file_row['acc_side']]) # [m/s2]
gyro_x_log = gyro_convert(all_log_original[:, 3+file_row['acc_axis']])  # [rad/s]
gyro_y_log = gyro_convert(all_log_original[:, 3+file_row['acc_lug']]) # [rad/s]
gyro_z_log = gyro_convert(all_log_original[:, 3+file_row['acc_side']])  # [rad/s]
